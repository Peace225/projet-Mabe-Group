...
import { Link } from 'react-router-dom'; // <--- L'IMPORT CORRIGÉ
...
export default function Events() {
  ...
  return (
    <div className="bg-[#FCFAF6] min-h-screen pt-40 pb-32 text-[#1A1A1A]">
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-32 text-center">
          <span className="text-[#C9A227] text-[10px] uppercase tracking-[0.5em] font-bold block mb-6">Agenda Corporate</span>
          <h1 className="text-5xl md:text-8xl font-black uppercase tracking-tighter mb-8 italic">Événements</h1>
          ...
          {eventsData.map((event) => (
            <div key={event.id} className="group bg-white border border-[#1A1A1A]/5 p-2 transition-all duration-700 hover:shadow-[0_20px_50px_rgba(0,0,0,0.05)]">
              <div className="relative aspect-[4/3] overflow-hidden mb-6">
                ...
                <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-md px-3 py-1 border border-[#1A1A1A]/10 text-[9px] uppercase tracking-[0.2em] font-bold">
                  {event.status}
                </div>
              </div>
              <div className="px-4 pb-6">
                <div className="flex justify-between items-start mb-4">
                  <span className="text-[9px] uppercase tracking-[0.2em] text-[#C9A227] font-bold">{event.category}</span>
                  <span className="text-[9px] uppercase text-gray-400">{event.date}</span>
                </div>
                <h3 className="text-lg font-bold uppercase tracking-tight mb-6 leading-tight group-hover:text-[#C9A227] transition-colors">
                  {event.title}
                </h3>
        ...
        <div className="border border-[#1A1A1A]/10 p-16 text-center hover:border-[#C9A227]/30 transition-all duration-500 bg-white">
          <h3 className="text-xl uppercase tracking-[0.3em] font-bold mb-6">Service sur-mesure</h3>
          ...
          <Link to="/contact" className="px-12 py-4 border border-[#1A1A1A] text-[10px] uppercase tracking-[0.3em] font-bold hover:bg-[#1A1A1A] hover:text-white transition-all">
            Nous consulter
          </Link>
        </div>
      </div>
